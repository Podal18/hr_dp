            ...
            if role == 3:  # HR
                ...
                try:
                    ...
                    with conn.cursor() as cursor:
                        # Исправленный запрос (по user_id)
                        ...
                        qtime = self.ui.time_input.time()
                        login_time = f"{qtime.hour():02d}:{qtime.minute():02d}:00"  # Исправленный формат
                        ...
                        if not cursor.fetchone():
                            # ВАЖНО: статус теперь устанавливает триггер БД!
                            cursor.execute("""
                                       INSERT INTO attendance (employee_id, date, login_time, notes)
                                       VALUES (%s, %s, %s, 'Автоматически создано при входе')
                                   """, (employee_id, current_date, login_time))
                            conn.commit()
